...
const Navbar = () => {
    ...
    const navItems =[
        ...
        {link: "CONTACT US", path: "https://github.com/yazdanhaider",target: "_blank"},
  ...
  return (
    <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300'>
        <nav className={`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 bg-green-100" : ""}`}>
            <div className='flex justify-between items-center text-base gap-8'>
                {/*logo*/}
                <Link to="/" className='text-2xl font-bold text-green-600 flex items-center gap-2'><FaPaintbrush className='inline-block' />Fusion Gallery</Link>
